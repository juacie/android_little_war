...
> 已修正：先前版本的預設陣容把槍兵放在 row1（中排），距離公式下 row1 攻擊者的最小可能距離是 2（`depthDistance = 1 + 0 + 1`），槍兵射程只有 1，永遠打不到任何目標。現已將全部射程 1 的近戰單位（盾兵／槍兵）改排在 row0 前排（見第 8 節新版陣容圖），確保每個單位都打得到人。
...
第一次修正後改為前排坦、中排輸出的配置，但中排槍兵（射程 1）一樣打不到人（`depthDistance` 最小是 2）；之後又發現這個問題並再次修正。目前版本把所有射程 1 的近戰單位都收進 row0 前排（前排對前排距離恰好是 1），中排只留射程 2 的領袖，後排維持射程 5 的遠程輸出：
